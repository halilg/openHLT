...
def list_all_modules(sequence):
    """Retun the orered list all modules in a sequence or path"""
    ...
    class ExtractInputTagVisitor(object):
        ...
        def enter(self, visitee):
            if visitee.isLeaf():
                ...
                self.inputs[label] = set( inputtag.split(':')[0] for inputtag in list_all_inputs( visitee ).values() )
...
def clone_path(process, path, process_changes):
    ...
    original_item = { label : process.__dict__[label]         for label in clones }
    cloned_item   = { label : process.__dict__[clones[label]] for label in clones }
    for sequence_label in affected_sequences:
